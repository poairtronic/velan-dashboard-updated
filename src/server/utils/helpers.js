...
async function getSheetData(sheetUrl) {
  const now = Date.now();
  if (state._cache.data && state._cache.url === sheetUrl && (now - state._cache.ts) < CACHE_TTL * 1000) {
    return { data: state._cache.data, cached: true, age: Math.round((now - state._cache.ts) / 1000) };
  }
