  ...
  return (
    <div>
      <div className="section-title">
        ⏱ Cycle Time <span>per Stage</span>
      ...
      >
        ℹ FORMULA: Avg days in stage = Mean of (Next Stage Timestamp − Current Stage Timestamp) for
        each item SC transition. Time component ignored — date only. This measures actual time spent
        in each stage from one timestamp to the next.
      </div>

      <div className="kpi-grid">
        <KPICard
          label="AVG OVERALL CYCLE"
          value={totalAvgCycle != null ? `${totalAvgCycle} days` : '—'}
          sub="avg from PO date to last timestamp date · date only"
          color1="#00c9ff"
          color2="#0fa8e0"
        />
        <KPICard
          ...
          sub={`~${slowestStage?.duration || 0} days avg at stage`}
          color1="#ff3d5a"
          color2="#ff6b35"
          badge={{ text: 'BOTTLENECK', cls: 'badge-red' }}
        />
        <KPICard
          label="FASTEST STAGE"
          value={fastestStage?.stage || '—'}
          sub={`~${fastestStage?.duration || 0} days avg at stage`}
          color1="#00e676"
          color2="#00c9ff"
          badge={{ text: 'QUICK', cls: 'badge-green' }}
        />
        <KPICard
          label="STAGES TRACKED"
          value={cts.length}
          sub="stages with cycle data"
          color1="#ffd60a"
          color2="#b24bff"
        />
        <KPICard
            ...
            <tbody>
              {[...cts]
                ...
                .map((c, i) => {
                  ...
                  const color =
                    pct >= 70 ? 'var(--danger)' : pct >= 40 ? 'var(--warning)' : 'var(--success)';
                  const rating = pct >= 70 ? '🔴 SLOW' : pct >= 40 ? '🟡 MODERATE' : '🟢 FAST';
                  const vsTarget =
                    c.avgToReach > TARGET_DAYS
                      ? `+${c.avgToReach - TARGET_DAYS}d over`
                      : `-${TARGET_DAYS - c.avgToReach}d under`;
                    ...
                    <tr key={i}>
                      <td>
                        <span
                          ...
                          style={{
                            background: getStageColor(c.stage) + '22',
