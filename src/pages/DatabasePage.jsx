  ...
  const filtered = React.useMemo(() => {
    const result = data.filter((row) => {
      ...
      if (
        filters.category &&
        getProductCategory &&
        getProductCategory(row.type) !== filters.category
      )
        return false;
      if (filters.search) {
    ...
    <div>
      ...
      <div className="chart-card" style={{ marginBottom: 16, background: 'var(--bg-card2)' }}>
        ...
        >
          ...
          >
            {filtered.length} filtered · {data.length} total
          </span>
        </div>
      </div>
          ...
          {(() => {
            ...
            Object.keys(groups).forEach((sc) => {
              ...
              normalized.forEach((r) => {
                ...
                const rD = isDoneStage(r.currentStage),
                  eD = isDoneStage(ex.currentStage);
                if (rD && !eD) {
                  dm[k] = r;
                  return;
                }
              ...
              <tbody>
                {(() => {
                  ...
                  const completedSCs = scsInView
                    ...
                    .filter(([, latestRows]) =>
                      latestRows.every((r) => isDoneStage(r.currentStage))
                    );
